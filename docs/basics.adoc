...
The script can also be run in the foreground by invoking `./bin/gremlin-server.sh` with an optional parameter of a Gremlin Server yaml file. When running JanusGraph as a daemon, it is expected that the operator will install/run the storage and indexing backends separately.
...
For System V you can copy `gremlin-server.sh` to `/etc/init.d/`. You will also need to copy `conf/janusgraph.env` to `/etc/default/janusgraph` and make sure that `$JANUSGRAPH_HOME` and `$JANUSGRAPH_YAML` are pointing at the correct locations.

**Note** This example assumes that `$JANUSGRAPH_HOME` is `/opt/janusgraph`
...
For System D there is a unit file provided at `conf/janusgraph.service` that can be copied to `/lib/systemd/system/`. This unit file expects an Environment variable definition file at `/opt/janusgraph/conf/janusgraph.env` that can be found at `conf/janusgraph.env` in the pre-packaged distribution. You can configure the `EnvironmentFile` location in the `janusgraph.service` file. All file locations used in `gremlin-server.sh` can be configured by Environment variables which should be defined in `janusgraph.env` if you wish to override the default locations.
...
[source, text]
----
cp conf/janusgraph.service /lib/systemd/system/
----
