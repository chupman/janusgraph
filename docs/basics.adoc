...
The JanusGraph https://github.com/JanusGraph/janusgraph/releases[release] also comes with an init script that can be found at `bin/gremlin-server.sh`. The `gremlin-server.sh` script was upgraded to an init script which supports start, stop, status. Its usage is backwards compatible and can still be run in the foreground by invoking `./bin/gremlin-server.sh` with an option parameter of a gremlin server yaml file.

If running JanusGraph as a daemon it is recommended to rename the pre-packaged to simply `janusgraph`, after unzipping, and move the distribution to `/opt/`.
