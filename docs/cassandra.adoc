...
. If you want to be able to run a Gremlin Server instance with the Cassandra container and plan on <<using_the_pre_packaged_distribution>> default configuration files using the you'll need to enable Cassandra Thrift. To do so open `conf/cassandra.yaml` and update `start_rpc: false` to `start_rpc: true`. If Cassandra is already running Thrift can be started manually with `bin/nodetool enablethrift`. the Thrift status can be verified with `bin/nodetool` statusthrift.
